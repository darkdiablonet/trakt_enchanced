...
import { headers as traktHeaders, deviceCode as traktDeviceCode, refreshToken as traktRefreshToken, historyChunk as traktHistoryChunk, enrichShowsWithProgress, showProgressWatched,  get as traktGet, loadToken as traktTokenLoad, saveToken as traktTokenSave } from './trakt.js';
...
export async function buildPageData(req, { forceRefreshOnce=false, allowFull=false } = {}) {
  ...
  if (!ACCESS_TOKEN && sessionTok?.refresh_token) {
    ...
    if (ref && ref.access_token) { req.session.trakt = ref; ACCESS_TOKEN = ref.access_token; await traktTokenSave(ref); }
  }
  if (!ACCESS_TOKEN) {
    const saved = await traktTokenLoad();
    if (saved?.access_token) { req.session.trakt = saved; ACCESS_TOKEN = saved.access_token; }
  }
  ...
  if (!ACCESS_TOKEN) {
    if (!req.session.device_code) {
      const dc = await traktDeviceCode(TRAKT_CLIENT_ID);
      if (dc?.device_code) req.session.device_code = { ...dc, _client_id:TRAKT_CLIENT_ID, _client_secret:TRAKT_CLIENT_SECRET };
    }
    devicePrompt = req.session.device_code || null;
  ...
  for (const s of master.shows) {
    ...
    showsRows.push({
      ids, title, year: y, episodes: watched, episodes_total: total,
      missing, watched_at: s.last_watched_at || null,
      poster: meta.poster || svgNoPoster(), trakt_url: slug ? `https://trakt.tv/shows/${slug}` : null, tmdb_url: meta.tmdbUrl || null
    });
  }
  ...
  for (const m of master.movies) {
    ...
    moviesRows.push({
      title, year: y, plays: Number(m.plays||0), watched_at: m.last_watched_at || null,
      poster: meta.poster || svgNoPoster(), trakt_url: slug ? `https://trakt.tv/movies/${slug}` : null, tmdb_url: meta.tmdbUrl || null
    });
  }
  ...
  const data = { showsRows, moviesRows, showsUnseenRows, moviesUnseenRows };
