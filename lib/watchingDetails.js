                ...
                for (const item of detailedHistory) {
                  ...
                  if (watchedAt) {
                    watchings.push({
                      ...
                      history_id: item.id // Ajouter l'ID d'historique pour suppression spécifique
                    });
                  }
                }
...
/**
 * Invalide le cache de watching details pour un élément spécifique
...
export function invalidateWatchingCache(traktId, kind) {
  // Pour le backend, on n'a pas de cache en mémoire comme le frontend
  // L'invalidation se fait en forçant une nouvelle récupération depuis l'API
  logger.debug(`WatchingDetails: Cache invalidated for ${kind} ${traktId} (backend)`);
}