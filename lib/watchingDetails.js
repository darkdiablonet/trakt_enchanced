...
import { get as traktGet } from './trakt.js';
...
export async function getMovieWatchingDetails(traktId) {
  try {
    ...
    // 1. Récupérer les données fraîches directement depuis l'API Trakt history
    try {
      logger.debug(`WatchingDetails: Fetching fresh history data for movie ${traktId} from API`);
      const historyData = await traktGet(`/users/me/history/movies?extended=full&limit=1000`);
      
      if (Array.isArray(historyData) && historyData.length > 0) {
        // Filtrer les entrées pour ce film spécifique
        const movieWatchings = historyData.filter(item => 
        ...
        for (const item of movieWatchings) {
          ...
          if (watchedAt) {
            watchings.push({
              ...
              title: item.movie?.title || 'Film inconnu',
              year: item.movie?.year,
              action: item.action // 'watch', 'checkin', etc.
            });
          }
        }
        
        logger.debug(`WatchingDetails: Trouvé ${watchings.length} visionnages pour le film ${traktId} depuis l'API`);
      } else {
        logger.debug(`WatchingDetails: Aucun historique trouvé pour le film ${traktId} depuis l'API`);
      }
    } catch (err) {
