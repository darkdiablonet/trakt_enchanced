...
export async function getMovieWatchingDetails(traktId) {
  try {
    ...
    // 1. Chercher dans l'historique détaillé pour récupérer les vraies dates
    ...
    try {
      ...
      logger.debug(`Impossible de lire history_movies.json, fallback vers cache:`, err.message);
      
      // Fallback: Chercher dans le cache général (méthode corrigée - moviesRows au lieu de movies)
      ...
      try {
        ...
        logger.debug(`Impossible de lire trakt_history_cache.json:`, err2.message);
      }
    }
