...
// Device code persistence helpers
const DEVICE_CODE_FILE = path.join(process.cwd(), 'data', '.device_code.json');

export async function saveDeviceCode(deviceCodeData) {
  try {
    await fsp.mkdir(path.dirname(DEVICE_CODE_FILE), { recursive: true });
    await jsonSave(DEVICE_CODE_FILE, {
      ...deviceCodeData,
      created_at: Date.now()
    });
  } catch (error) {
    console.warn('[device_code] Failed to save:', error.message);
  }
}

export async function loadDeviceCode() {
  try {
    const data = await jsonLoad(DEVICE_CODE_FILE);
    if (!data) return null;
    
    // Expire après 10 minutes (device codes Trakt expirent généralement après 15min)
    const age = (Date.now() - (data.created_at || 0)) / 1000;
    if (age > 600) {
...
export async function clearDeviceCode() {
  try {
    await fsp.unlink(DEVICE_CODE_FILE);
  } catch {
...
// Get public host with intelligent defaults for different environments
export async function getPublicHost(port) {
  if (process.env.PUBLIC_HOST) {
    return process.env.PUBLIC_HOST;
  }
  
  // Try to detect if we're in Docker (common indicator files/env vars)
  let isDocker = false;
  try {
    isDocker = process.env.container || 
               process.env.HOSTNAME?.startsWith('docker-') ||
               (await fsp.access('/.dockerenv').then(() => true).catch(() => false));
  } catch {
    isDocker = false;
  }
  
  if (isDocker) {
    // In Docker, try to get the real host IP from common sources
    const dockerHost = process.env.DOCKER_HOST_IP || 
                       process.env.HOST_IP || 
                       '0.0.0.0';  // Fallback to bind all interfaces
    return `${dockerHost}:${port}`;
  }
