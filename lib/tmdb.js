...
import { TMDB_API_KEY, CACHE_DIR, IMG_DIR, LANGUAGE } from './config.js';
...
async function httpGetJson(url, headers={}) {
  ...
  try {
    ...
    loggers.logApiCall('tmdb', 'GET', url, duration, res.status);
    ...
    const duration = Date.now() - startTime;
    loggers.logApiCall('tmdb', 'GET', url, duration, 0, err);
    return null;
  }
}

// Fonction pour nettoyer le code de langue pour l'API TMDB
function cleanLanguageCode(lang) {
  if (!lang) return 'fr-FR';
  
  // Supprimer .UTF-8 et autres suffixes
  const cleaned = lang.replace(/\.UTF-8|\.utf8/gi, '');
  
  // Mapper certaines valeurs communes
  const mapping = {
    'fr': 'fr-FR',
    'en': 'en-US',
    'fr_FR': 'fr-FR',
  ...
  return mapping[cleaned] || cleaned || 'fr-FR';
}

export async function tmdbGet(kind, id) {
  if (!TMDB_API_KEY) return null;
  const cleanLang = cleanLanguageCode(LANGUAGE);
  return httpGetJson(`https://api.themoviedb.org/3/${kind}/${id}?api_key=${TMDB_API_KEY}&language=${cleanLang}`);
}
export async function tmdbSearch(kind, query, year, language) {
  if (!TMDB_API_KEY || !query) return null;
  const y = year ? `&year=${encodeURIComponent(String(year))}` : '';
  // Utiliser la langue fournie en paramètre, ou celle de la config par défaut
  const cleanLang = cleanLanguageCode(language || LANGUAGE);
  return httpGetJson(`https://api.themoviedb.org/3/search/${kind}?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(query)}${y}&language=${cleanLang}`);
}

// Remplace intégralement posterLocalUrl par ceci :
export async function posterLocalUrl(req, posterPath, size = 'w342', traktId = null) {
  if (!posterPath) return null;

  // Si on a un traktId, l'utiliser comme nom de fichier, sinon utiliser l'ancien système
  const filename = traktId 
    ? `trakt_${traktId}.jpg`  // Nouveau format : trakt_12345.jpg
    : `${size}_${String(posterPath).replace(/^\//, '').replace(/\//g, '_')}`;  // Ancien format
    
  const file = path.join(IMG_DIR, filename);
  const relUrl = `/cache_imgs/${filename}`;
...
// Nouvelle fonction pour obtenir l'URL du poster depuis l'ID Trakt uniquement
export async function posterFromTraktId(traktId) {
  if (!traktId) return null;
  
  const filename = `trakt_${traktId}.jpg`;
  const file = path.join(IMG_DIR, filename);
  const relUrl = `/cache_imgs/${filename}`;
...
export async function getCachedMeta(req, kind, title, year, tmdbId, size, traktId = null) {
  let js = null;
  ...
  if (js?.poster_path) {
    poster = (await posterLocalUrl(req, js.poster_path, size, traktId))
  ...
  // 5) titre localisé depuis TMDB
  const tmdbTitle = kind === 'tv' 
    ? (js?.name || js?.original_name) 
    : (js?.title || js?.original_title);

  return { poster, tmdbUrl, overview, title: tmdbTitle };
}
