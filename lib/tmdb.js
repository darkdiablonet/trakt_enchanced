...
export async function tmdbGet(kind, id) {
  ...
  return httpGetJson(`https://api.themoviedb.org/3/${kind}/${id}?api_key=${TMDB_API_KEY}&language=${LANGUAGE}`);
}
export async function tmdbSearch(kind, query, year) {
  ...
  return httpGetJson(`https://api.themoviedb.org/3/search/${kind}?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(query)}${y}&language=${LANGUAGE}`);
}
