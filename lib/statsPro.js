// lib/statsPro.js (ESM)
import { get as traktGet, headers as traktHeaders, loadToken, hasValidCredentials } from './trakt.js';
import { getDetailsCached } from './tmdb.js';
import { TRAKT_CLIENT_ID } from './config.js';
...
async function fetchHistoryRange(pathType, startISO, endISO) {
  if (!hasValidCredentials()) {
    throw new Error('Missing Trakt credentials. Please configure TRAKT_CLIENT_ID and TRAKT_CLIENT_SECRET.');
  }
  
  // Charger le token pour faire des appels HTTP directs
  ...
  for (;;) {
	...
	try {
	  const url = `https://api.trakt.tv${base}?${qs}`;
	  ...
	  console.error(`[fetchHistoryRange] Error fetching ${pathType} page ${page}:`, error.message);
	  throw error;
	}
  }
  return out;
...
export async function computeStatsPro({ range='lastDays', year, lastDays=365, type='all', token } = {}) {
  // Le token n'est plus nécessaire car l'authentification est gérée automatiquement
  ...
  if (type === 'all' || type === 'movies') {
	chunks.push(await fetchHistoryRange('movies', startISO, endISO));
  }
  if (type === 'all' || type === 'shows') {
	chunks.push(await fetchHistoryRange('episodes', startISO, endISO)); // séries = épisodes
  }
