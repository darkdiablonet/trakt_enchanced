...
async function updateCachesOnChange() {
  ...
  try {
    ...
    // Get recent history (last 24 hours to be safe)
    ...
    if (hasNewItems) {
      ...
      try {
        ...
        // Delete specific show progress caches for affected shows first
        for (const item of recentHistory) {
          if (item.type === 'episode' && item.show?.ids?.trakt) {
            const progressFile = path.join(
              path.dirname(HIST_FILE), 
              'progress', 
              `watched_${item.show.ids.trakt}.json`
            );
            
            try {
              await fs.unlink(progressFile);
              console.log(`[monitor] Invalidated progress cache for show ${item.show.ids.trakt}`);
            } catch (err) {
        ...
        // Force a full rebuild in background - but wait for completion to track status properly
        await buildPageData(fakeReq, { forceRefreshOnce: true, allowFull: false });
 ...
 */
export function startActivityMonitor(intervalMs = 30000) {
  if (monitorInterval) {
