...
import { buildPageDataGranular } from './pageDataNew.js';
import { logger } from './logger.js';
import fs from 'node:fs/promises';
import path from 'node:path';
import { HIST_FILE, PAGE_CACHE_FILE } from './config.js';
import { updateSpecificCard } from './pageDataNew.js';
import { headers, loadToken } from './trakt.js';
import { invalidateWatchingCache } from './watchingDetails.js';
...
/**
 * Définit les fonctions de broadcast pour les mises à jour live
 */
export function setBroadcastFunction(broadcastFn, broadcastEventFn = null) {
  broadcastCardUpdate = broadcastFn;
  broadcastEvent = broadcastEventFn;
  console.log('[monitor] Broadcast functions set for live updates');
}
...
async function updateCachesOnChange() {
  ...
  try {
    ...
    // Checking history since timestamp AND checking for deletions
    ...
    // Also check for deletions by comparing current watched state
    // This is needed because deletions don't appear in recent history
    let watchedStateChanged = false;
    try {
      const { get: traktGet } = await import('./trakt.js');
      
      // Get current watched movies to detect deletions
      const currentWatchedMovies = await traktGet('/sync/watched/movies?extended=minimal');
      const currentWatchedShows = await traktGet('/sync/watched/shows?extended=minimal');
      
      // Simple state comparison - store checksums to detect changes
      const currentMovieCount = Array.isArray(currentWatchedMovies) ? currentWatchedMovies.length : 0;
      const currentShowCount = Array.isArray(currentWatchedShows) ? currentWatchedShows.length : 0;
      
      // Check if we have previous counts to compare
      if (global.lastWatchedCounts) {
        if (currentMovieCount !== global.lastWatchedCounts.movies || 
            currentShowCount !== global.lastWatchedCounts.shows) {
          watchedStateChanged = true;
          console.log(`[monitor] 🔍 Watched state changed: Movies ${global.lastWatchedCounts.movies}→${currentMovieCount}, Shows ${global.lastWatchedCounts.shows}→${currentShowCount}`);
        }
      }
      
      // Store current counts for next check
      global.lastWatchedCounts = { movies: currentMovieCount, shows: currentShowCount };
      
    } catch (watchedCheckError) {
      console.warn('[monitor] Failed to check watched state:', watchedCheckError.message);
    }
    ...
    if (hasNewItems || watchedStateChanged) {
          ...
          if (item.type === 'episode' && item.show) {
            ...
            if (traktId && !updatedItems.has(`show-${traktId}`)) {
              ...
              try {
                ...
                // Invalider le cache watching details pour cette série
                ...
                if (updatedCard) {
                  ...
                  if (broadcastEvent) {
                    broadcastEvent('invalidate-watching-cache', traktId, { kind: 'show' });
                  }
            ...
            if (traktId && !updatedItems.has(`movie-${traktId}`)) {
              ...
              try {
                ...
                // Invalider le cache watching details pour ce film
                ...
                if (updatedCard) {
                  ...
                  // Broadcaster l'invalidation du cache watching details au frontend
                  if (broadcastEvent) {
                    broadcastEvent('invalidate-watching-cache', traktId, { kind: 'movie' });
                  }
      ...
      // Si on a détecté un changement d'état mais qu'il n'y a pas d'éléments spécifiques
      // dans l'historique récent, invalider tous les caches watching details
      if (watchedStateChanged && recentHistory.length === 0) {
        console.log('[monitor] 🧹 Global watched state changed but no specific items detected - invalidating all watching caches');
        
        // Broadcast global cache invalidation
        if (broadcastEvent) {
          broadcastEvent('invalidate-all-watching-cache', null, { reason: 'global_state_change' });
        }
      }
      ...
      try {
        // Get valid token and headers for buildPageDataGranular
        const token = await loadToken();
        if (!token?.access_token) {
          console.warn('[monitor] No valid token available - skipping background rebuild');
          return;
        }
        
        const traktHeaders = headers(token.access_token);
        if (!traktHeaders) {
          console.warn('[monitor] Invalid headers - skipping background rebuild');
          return;
        }
        ...
        // Use the new granular system instead of the old buildPageData
        await buildPageDataGranular(traktHeaders);
        ...
        logger.error('Activity monitor background rebuild failed', { error: error.message });
      }
    } else {
      // No new items to add
    }
