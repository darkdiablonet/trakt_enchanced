...
export async function removeMovieFromHistory(movie) {
  try {
    let body;
    
    // Si on a un history_id, supprimer l'entrée spécifique
    if (movie.history_id) {
      body = {
        ids: [movie.history_id]
      };
    } else {
      // Sinon, supprimer toutes les entrées du film (comportement legacy)
      body = {
        movies: [{
          ids: { trakt: movie.trakt_id }
        }]
    ...
    if (result?.deleted?.movies > 0) {
      const movieTraktId = movie.trakt_id;
      if (movieTraktId) {
        try {
          await invalidateCache(['recent-movies', 'stats', 'calendar', `movie-${movieTraktId}`]);
        } catch (cacheErr) {
          loggers.logError(cacheErr, { operation: 'removeMovieFromHistory-cache', movie });
        }
      }
    }
    ...
    loggers.logError(error, { operation: 'removeMovieFromHistory', movie });
    throw error;
  }
}

/**
 * Add a movie or show to Trakt history with custom watched date/time
 * @param {Object} item - Item details
 * @param {string} item.type - 'movie' or 'show'
 * @param {string} item.title - Title of the item
 * @param {number|null} item.year - Year of release
 * @param {number|null} item.tmdb_id - TMDB ID
 * @param {string} item.watched_at - ISO datetime when item was watched
 * @returns {Promise<Object>} Trakt API response
 */
export async function addToHistory(item) {
  try {
    const { type, title, year, tmdb_id, watched_at } = item;
    
    if (!type || !title || !watched_at) {
      throw new Error('Missing required fields: type, title, watched_at');
    }
    ...
    // Prepare the payload for Trakt sync/history API
    const payload = {};
    ...
    if (type === 'movie') {
      ...
      if (tmdb_id) {
        payload.movies[0].ids = { tmdb: tmdb_id };
      }
    } else {
      // For shows, we add the entire show as watched
      payload.shows = [{
        title: title,
        year: year || null,
        watched_at: watched_at
      ...
      if (tmdb_id) {
        payload.shows[0].ids = { tmdb: tmdb_id };
      }
    }
    
    console.log('[trakt] Adding to history:', JSON.stringify(payload, null, 2));
    
    const result = await makeAuthenticatedCall('/sync/history', payload, 'POST');
    
    console.log('[trakt] Add to history response:', JSON.stringify(result, null, 2));
    
    return result;
  } catch (error) {
    loggers.logError(error, { operation: 'addToHistory', item });
    throw error;
  }
}
 ...
 */
/**
 * Force la création du cache global watched_shows_complete.json
 * Utilisé par la heatmap et d'autres composants qui ont besoin de toutes les données
 */
export async function ensureWatchedShowsCache() {
  const CACHE_FILE = path.join(DATA_DIR, '.cache_trakt', 'watched_shows_complete.json');
  
  // Vérifier si le cache existe déjà et n'est pas trop vieux
  try {
    const st = await fsp.stat(CACHE_FILE).catch(() => null);
    const fresh = st ? ((Date.now() - st.mtimeMs) / 1000) < PROG_TTL_SECS : false;
    
    if (fresh) {
      console.log('[progress] Global watched cache already fresh');
      return;
    }
  } catch (err) {
  ...
  console.log('[progress] Creating/refreshing global watched shows cache...');
  
  try {
    // Récupérer toutes les données watched depuis l'API
    const watchedShows = await get('/sync/watched/shows');
    
    if (Array.isArray(watchedShows)) {
      console.log(`[progress] fetched ${watchedShows.length} watched shows for global cache`);
      // Créer le dossier si nécessaire
      await fsp.mkdir(path.dirname(CACHE_FILE), { recursive: true }).catch(() => {});
      // Sauvegarder en cache
      await jsonSave(CACHE_FILE, watchedShows);
      console.log('[progress] Global watched cache created successfully');
    } else {
      console.warn('[progress] unexpected API response format for watched shows');
    }
  } catch (error) {
    console.error('[progress] ❌ FAILED to create global watched cache:', error.message);
    console.error('[progress] ❌ Full error:', error);
    throw error;
  }
}
