...
import { invalidatePageCache } from './pageData.js';
...
export async function enrichShowsWithProgressOptimized(rows, { updateMissing = true } = {}) {
  ...
  await fetchNextEpisodesForIncompleteSeries(rows);
}
...
async function fetchNextEpisodesForIncompleteSeries(rows) {
  // Identifier les séries qui pourraient avoir des épisodes suivants
  // Cela inclut : missing > 0 OU séries sans données next_episode
  const incompleteSeries = rows.filter(row => {
    ...
    // Récupérer les données si : missing > 0 OU pas encore de données next_episode
    return traktId && (missing > 0 || !hasNextEpisodeData);
  });
...
export async function removeEpisodeFromHistory(episode) {
  try {
    ...
    // Si succès, invalider le cache ET forcer le refresh des données de progression
    if (result?.deleted?.episodes > 0) {
      await invalidateProgressCache(episode.trakt_id);
      
      // Forcer le refresh des données de progression pour recalculer next_episode
      try {
        await forceRefreshProgressCache(episode.trakt_id);
      } catch (error) {
        console.warn(`[progress] Failed to force refresh after unmark:`, error.message);
      }
      
      // CRUCIAL: Invalider le cache page pour forcer le rechargement des boutons next
      try {
        await invalidatePageCache();
        console.log(`[trakt] Page cache invalidated after episode unmark`);
      } catch (error) {
        console.warn(`[trakt] Failed to invalidate page cache after unmark:`, error.message);
      }
    }
