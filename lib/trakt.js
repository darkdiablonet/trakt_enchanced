  ...
  try {
    ...
    if (tokenData && tokenData.encrypted) {
      ...
      try {
        ...
        console.warn('[trakt] Failed to decrypt old token, treating as invalid');
        return null;
      }
    }
...
export async function ensureValidToken(forceRefresh = false) {
  ...
  if (!token.refresh_token) {
    ...
    return token; // Return existing token even if expiring
  }
  ...
  try {
    ...
    return token; // Return old token on error
  }
}