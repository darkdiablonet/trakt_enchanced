...
export function listTable(rows, {cols=[['name','Nom'],['minutes','Min'],['plays','Vus']], limit=10} = {}){
  ...
  const body = rows.slice(0, limit).map(r=>{
    return `<tr class="border-b border-white/5">
      ${cols.map(([k])=>`<td class="py-1 pr-3">${(k in r)?(typeof r[k]==='number'?r[k].toLocaleString(locale):String(r[k])):''}</td>`).join('')}
    </tr>`;
  }).join('');
...
export async function renderStatsPro(data){
  ...
  sumEl.innerHTML = `
    <div class="glass rounded-xl p-3 animate-fade-in-up hover:scale-105 transition-transform cursor-pointer delay-200">
      <div class="text-xs text-muted">${i18n.t('stats.watched')}</div>
      <div class="text-2xl font-semibold animate-count-up delay-600">${(T.plays||0).toLocaleString(locale)}</div>
    </div>
    <div class="glass rounded-xl p-3 animate-fade-in-up hover:scale-105 transition-transform cursor-pointer delay-400">
      <div class="text-xs text-muted">${i18n.t('stats.movies')}</div>
      <div class="text-2xl font-semibold animate-count-up delay-800">${(T.movies||0).toLocaleString(locale)}</div>
    </div>
    <div class="glass rounded-xl p-3 animate-fade-in-up hover:scale-105 transition-transform cursor-pointer delay-600">
      <div class="text-xs text-muted">${i18n.t('stats.episodes')}</div>
      <div class="text-2xl font-semibold animate-count-up delay-1000">${(T.episodes||0).toLocaleString(locale)}</div>
    </div>
    <div class="glass rounded-xl p-3 animate-fade-in-up hover:scale-105 transition-transform cursor-pointer delay-800">
      <div class="text-xs text-muted">${i18n.t('stats.hours')}</div>
      <div class="text-2xl font-semibold animate-count-up delay-1200">${(T.hours||0).toLocaleString(locale)}</div>
    </div>
    <div class="glass rounded-xl p-3 animate-fade-in-up hover:scale-105 transition-transform cursor-pointer delay-1000">
      <div class="text-xs text-muted">${i18n.t('stats.active_days')}</div>
    ...
    <div class="glass rounded-xl p-3 animate-fade-in-up hover:scale-105 transition-transform cursor-pointer delay-1200">
      <div class="text-xs text-muted">${i18n.t('stats.max_per_day')}</div>
...
// Variable pour stocker les dernières données Pro Stats
let lastProStatsData = null;
