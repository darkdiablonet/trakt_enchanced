...
export class LazyLoadManager {
  ...
  loadImage(element) {
    ...
    // Set the background using CSS variable
    element.style.setProperty('--bg-image', `url('${bgUrl}')`);
    element.classList.add('loaded', 'bg-image-dynamic');
  ...
  convertExistingImages() {
    ...
    document.querySelectorAll('.poster[style*="background-image"]').forEach(poster => {
      ...
      if (match) {
        ...
        poster.style.removeProperty('background-image');
...
export function fallbackImageLoading() {
  setTimeout(() => {
    document.querySelectorAll('.poster[data-bg-src]').forEach(element => {
      ...
      if (bgUrl) {
        element.style.setProperty('--bg-image', `url('${bgUrl}')`);
        element.classList.add('bg-image-dynamic');
      }
    });
  }, 100);
}
